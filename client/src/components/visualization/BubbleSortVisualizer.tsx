...
function getBubbleSortSteps(input: number[]): Step[] {
  const arr = [...input];
  const steps: Step[] = [{ array: [...arr], comparing: null, swapped: false }];
  const n = arr.length;
  for (let i = 0; i < n - 1; i++) {
  ...
  steps.push({ array: [...arr], comparing: null, swapped: false });
  return steps;
}

const DEFAULT_INPUT = [6, 5, 3, 1, 8, 7, 2, 4];

const BubbleSortVisualizer: FC = () => {
  const [input, setInput] = useState(DEFAULT_INPUT.join(','));
  const [steps, setSteps] = useState<Step[]>(getBubbleSortSteps(DEFAULT_INPUT));
  ...
  const handleSetArray = () => {
    const arr = input
      ...
      .filter((x) => !isNaN(x));
    if (arr.length > 1) {
      setSteps(getBubbleSortSteps(arr));
      setCurrent(0);
      setPlaying(false);
    }
  };
