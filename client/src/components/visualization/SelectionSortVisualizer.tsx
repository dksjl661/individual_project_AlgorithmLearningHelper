...
function getSelectionSortSteps(input: number[]): Step[] {
  const arr = [...input];
  const steps: Step[] = [{ array: [...arr], comparing: null, minIndex: -1, swapped: false }];
  const n = arr.length;
  for (let i = 0; i < n - 1; i++) {
    let minIndex = i;
    for (let j = i + 1; j < n; j++) {
      steps.push({ array: [...arr], comparing: [minIndex, j], minIndex, swapped: false });
    ...
    if (minIndex !== i) {
      ...
      steps.push({ array: [...arr], comparing: [i, minIndex], minIndex, swapped: true });
    }
  }
  steps.push({ array: [...arr], comparing: null, minIndex: -1, swapped: false });
  return steps;
}

const DEFAULT_INPUT = [6, 5, 3, 1, 8, 7, 2, 4];

const SelectionSortVisualizer: FC = () => {
  const [input, setInput] = useState(DEFAULT_INPUT.join(','));
  const [steps, setSteps] = useState<Step[]>(getSelectionSortSteps(DEFAULT_INPUT));
    ...
    <div className="w-full max-w-2xl mx-auto p-6 card mt-8">
      ...
      <div className="flex items-end justify-center space-x-2 h-40 mb-4">
        {step.array.map((num: number, idx: number) => {
          ...
          const isMinIndex = idx === step.minIndex;
